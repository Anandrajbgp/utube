...
class NewVersionWorker(
    ...
    private fun checkNewVersion() {
        ...
        val updateApiUrl = applicationContext.getString(R.string.update_api_url)
        if (!BuildConfig.DEBUG && updateApiUrl.contains("YOUR-UTUBE-UPDATE-HOST")) {
            return
        }
        ...
        val response = DownloaderImpl.getInstance().get(updateApiUrl)
    ...
    private fun handleResponse(response: Response) {
        ...
        try {
            val root = JsonParser.`object`().from(response.responseBody())
            val newpipeVersionInfo = if (root.has("version")) {
                root
            } else {
                root.getObject("flavors").getObject("newpipe")
            }
